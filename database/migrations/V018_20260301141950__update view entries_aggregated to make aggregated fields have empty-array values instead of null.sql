...
SELECT 
    e.*,
    COALESCE(c.categories, JSON_ARRAY()) AS categories,
    COALESCE(x.examples, JSON_ARRAY()) AS examples,
    COALESCE(f.forms, JSON_ARRAY()) AS forms,
    COALESCE(m.meanings, JSON_ARRAY()) AS meanings,
    COALESCE(s.sources, JSON_ARRAY()) AS sources
FROM entries e
LEFT JOIN (
